...
function checkin() {
    ...
    var sendemail = 1; // to control email send in testing
    var sendsms = 1; // to control sendsms in testing
    ...
    $.post("/CheckIn", obj, function (data, status) {
        ...
        if (data.done == 1) {
            ...
            $.post("/sendemail", body, function (info, status2) {
                alert(info);
                //condition to control sending sms
                if (sendsms === 1) {
                    $.post("/sendsms", {
                        tosend: data.hostphone,
                        message: mssg
                    }, function (infoq, statusq) {
                        if (status == 200)
